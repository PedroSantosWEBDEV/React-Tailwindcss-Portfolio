...
test('it shows the title in the banner', () => {
	setup();
	// We expect that the title 'Hi, I'm Pedro Vinicius' is in the banner component
	expect(screen.getByText(/Hi, I'm Pedro Vinicius/i)).toBeInTheDocument();
});
...
test('can download cv when clicked on download cv button', async () => {
	...
	// expect(downloadLink.download).toEqual('Pedro-Resume.pdf');
	// expect(downloadLink.href).toEqual('/files/Pedro-Resume.pdf');
